...
echo "loose-group_replication_local_address=$hostname:24901" >> /etc/my.cnf

if [ -n "$ipaddr" ]; then
        echo "loose-group-replication-ip-whitelist=$ipaddr/24" >> /etc/my.cnf
else
        # this is needed for docker --net=host
        # get list of ip addresses 
        listip=`ip addr | grep 'state UP' -A2 | grep -P "inet\s" |  awk '{print $2}'  | paste -sd "," -`
        echo "loose-group-replication-ip-whitelist=$listip" >> /etc/my.cnf
fi
...
if [ -n "$CLUSTER_JOIN" ]; then
        ...
        echo "loose-group_replication_group_seeds=$cluster_join:24901" >> /etc/my.cnf
fi
